...
export default function Sidebar({ collapsed, setCollapsed, mobileOpen, setMobileOpen }) {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside
        className={`hidden md:flex flex-col h-screen bg-white dark:bg-[#0f0b1e] border-r border-slate-200/60 dark:border-violet-800/20 relative z-30 flex-shrink-0 ${collapsed ? 'w-[72px]' : 'w-[260px]'}`}
      >
        <SidebarContent />
      ...
      {mobileOpen && (
        <>
          ...
          />
          <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white dark:bg-[#0f0b1e] z-50 md:hidden">
            <button
              onClick={() => setMobileOpen(false)}
