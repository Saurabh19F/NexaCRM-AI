...
function IPWhitelistPanel() {
  ...
  return (
    <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }}
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl">
        <div className="flex items-center gap-2">
          ...
          <AnimatePresence>
            {ips.map(entry => (
              <motion.div key={entry.id} initial={{ opacity:0, x:-10 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:10 }}
                className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0 group">
        ...
        <div className="w-48 flex-shrink-0">
          <nav className="space-y-0.5">
            {TABS.map(({ key, label, icon: Icon }) => (
              <button key={key} onClick={() => setTab(key)} className={`nav-item w-full ${tab === key ? 'active' : ''}`}>
          ...
          {tab === 'notifications' && (
            <div className="space-y-5">
              ...
              {[
                ...
                <div key={label} className="flex items-center justify-between py-3 border-b border-slate-200/60 dark:border-slate-700/40">
                ...
                <div className="flex gap-3">
                  {[
                    ...
                    <button key={label} onClick={fn}
                      className={`flex items-center justify-center gap-2 px-8 py-6 rounded-2xl border-2 text-sm font-semibold transition-all
