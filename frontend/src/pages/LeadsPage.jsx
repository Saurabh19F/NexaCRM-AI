...
export default function LeadsPage() {
  const { leads, replaceLeads } = useLeadsStore()
  ...
  const setLeads = (updater) => {
    const current = useLeadsStore.getState().leads
  ...
  const handleAdd    = (lead) => setLeads(prev => [lead, ...prev])
  const handleSave   = (updated) => {
    const nowIso = new Date().toISOString()
    setLeads((prev) => prev.map((l) => {
      if (l.id !== updated.id) return l
      const next = { ...updated, lastActivityAtTs: nowIso }
      const wasNew = String(l.status ?? '').toLowerCase() === 'new'
      const isNowNew = String(updated.status ?? '').toLowerCase() === 'new'
      if (wasNew && !isNowNew && !l.firstResponseAtTs) {
        next.firstResponseAtTs = nowIso
      }
      return next
    }))
  }
  ...
  const handleExport = () => {
    const cols = ['name','email','phone','company','source','score','status','value','assignedTo','createdAt']
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const imported = lines.slice(1).map((row, i) => {
        const vals = row.split(',').map(v => v.trim().replace(/"/g,''))
        const obj = {}; headers.forEach((h, j) => obj[h] = vals[j] ?? '')
        const parsedCreated = obj.createdAt ? new Date(obj.createdAt) : null
        const createdAtTs = parsedCreated && !Number.isNaN(parsedCreated.getTime())
        ...
        return {
          ...
          followUpSlaMinutes: Number(obj.followUpSlaMinutes) > 0 ? Number(obj.followUpSlaMinutes) : 60,
          createdAtTs,
        }
      }).filter(l => l.name)
      setLeads(prev => [...imported, ...prev])
      toast.success(`${imported.length} lead(s) imported.`)
    }
            ...
            <thead>
              <tr className="border-b border-slate-200/60 dark:border-slate-700/40 bg-slate-50/50 dark:bg-slate-800/30">
                ...
                {[
                  ...
                  <th key={key} className="py-3 px-4 text-left font-semibold text-slate-600 dark:text-slate-400 cursor-pointer hover:text-slate-800 dark:hover:text-slate-200"
                ...
                {filtered.map((lead) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div>
                          <p className="font-semibold text-slate-800 dark:text-slate-200">{lead.name}</p>
                      ...
                      <td className="py-3 px-4 text-slate-600 dark:text-slate-400 text-xs">{lead.assignedTo}</td>
                      <td className="py-3 px-4 text-slate-500 text-xs">{lead.createdAt}</td>
                      <td className="py-3 px-4">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setEditLead(lead)}
                            className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors"
                            title="Edit lead">
                            <Edit className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => setHistoryLead(lead)}
                            className="p-1.5 rounded-lg hover:bg-violet-50 dark:hover:bg-violet-950/20 text-slate-400 hover:text-violet-500 dark:hover:text-violet-400 transition-colors"
                            title="View history">
                            <History className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => setActivitiesLead(lead)}
                            className="p-1.5 rounded-lg hover:bg-brand-50 dark:hover:bg-brand-950/20 text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors"
                            title="Lead Activities">
                            <ClipboardList className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => setWaLead(lead)}
                            className="p-1.5 rounded-lg hover:bg-green-50 dark:hover:bg-green-950/20 text-slate-400 hover:text-green-600 dark:hover:text-green-400 transition-colors"
                            title="Send WhatsApp">
